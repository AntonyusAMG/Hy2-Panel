...
**Версия агента / UI:** см. `node-agent/VERSION` (текущая **1.1.1**).
...
- **Срок подписки (интеграция с ботом):** `GET /users` отдаёт `expires` из `config.json` → `user_expires`; `PATCH /users/{telegram_id}/expire` с телом `{"expire_unix": N}` (0 = безлимит). В панели — колонка «Истекает».

### 🎨 Современный Web UI
- **Glass / dock**: Верхняя и нижняя панели, адаптивная вёрстка, настройки размытия.
- **Трафик за N дней**: Суточная статистика на стороне ноды (`traffic_daily.json`), API `GET /traffic/daily`.
- **Многотемность**: Светлая / тёмная и акценты.
